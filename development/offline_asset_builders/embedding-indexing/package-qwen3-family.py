...
# ruff: noqa
# %% [markdown]
# # Package Qwen3 Embedding & Reranker Suite (~5.2 GB)
#
# Dedicated asset builder packaging the modern Qwen3 IR family:
# 1. `Qwen/Qwen3-Embedding-0.6B` (PyTorch safetensors, 1024-d, ~1.2 GB)
# 2. `Qwen/Qwen3-Embedding-0.6B-GGUF` (single Q4_K_M quant, ~400 MB)
# 3. `Qwen/Qwen3-Embedding-4B-GGUF` (single Q4_K_M quant, ~2.4 GB)
# 4. `Qwen/Qwen3-Reranker-0.6B` (PyTorch safetensors reranker, ~1.2 GB)
# + Offline Python wheels (`sentence-transformers`, `faiss-cpu`, `rank-bm25`, etc.)
#
# Run on Kaggle (CPU, Internet ON). Saves as `qwen3-embedding-reranker-assets`.
